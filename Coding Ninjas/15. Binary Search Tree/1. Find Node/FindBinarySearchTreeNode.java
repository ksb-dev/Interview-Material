...
public class FindBinarySearchTreeNode {

	public static boolean searchInBST(BinaryTreeNode<Integer> node, int key) {

		if (node == null) return false;

		if (node.data == key) return true;

		boolean res1 = searchInBST(node.left, key);

		if(res1) return true;

		boolean res2 = searchInBST(node.right, key);
